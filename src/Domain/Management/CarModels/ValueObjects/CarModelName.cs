...
namespace Domain.Management.CarModels.ValueObjects;
...
public sealed class CarModelName : ValueObject
{
    ...
    public static Result<CarModelName> Create(string carModelName)
    {
        ...
        {
            return Result.Failure<CarModelName>(DomainErrors.CarModel.CarModelNameTooLong);

        }
        else if (carModelName.Length < MinLength)
        {
            return Result.Failure<CarModelName>(DomainErrors.CarModel.CarModelNameTooShort);
